...
* **Per-segment timing + repeats** – Each chord segment has a `Start` beat selector (`1st`–`4th`) plus independent `Chord x`, `Bass x`, and `String x` repeat selectors (`1`, `2`, `4`, `8`, `16`, `32`) so each voice can enter later within the block while still subdividing repeats independently.
