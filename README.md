...
* **2-oscillator chord + bass synths** – Separate chord and bass synth panels with presets, Osc 1/Osc 2 waveform selection, per-synth transpose, ADSR, multi-pole filter slope/cutoff/resonance, tempo-synced delay (time + straight/dotted/triplet feel + mix/filter/amount), harmonic drive (distortion), simple modulation (rate/depth vibrato), compact per-synth LFO target controls for cutoff/Q/decay, and extra tone controls (Osc2 interval, detune, mix), plus bass enable/disable.
