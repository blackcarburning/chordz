...
* **Per-segment timing + repeats** – Each chord segment has a `Start` beat selector (`1st`–`4th`) plus independent `Chord x` and `Bass x` repeat selectors (`1`, `2`, `4`) so chord and bass playback can enter later within the block while still subdividing repeats independently.
* **2-oscillator chord + bass synths** – Separate chord and bass synth panels with presets, Osc 1/Osc 2 waveform selection, per-synth transpose, ADSR, filter cutoff/resonance, harmonic drive (distortion), simple modulation (rate/depth vibrato), and extra tone controls (Osc2 interval, detune, mix), plus bass enable/disable.
* **Per-chord cycle/arpeggiate toggle** – Every chord card has a `↻ On/Off` control so one chord can arpeggiate while others remain block chords.
